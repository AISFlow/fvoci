...
| `FVOCI_UPLOAD_MAX_CONCURRENT_PARTS` | Part PUTs in flight per server process (default 64, positive). Each holds an inbound connection and, with S3, an outbound one while the client streams its body. When every slot is taken, a part PUT is refused before its body is read with `503` problem `upload_capacity_exceeded` and `Retry-After: 2`; the web client retries 5xx part uploads with backoff. |
