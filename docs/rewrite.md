...
최신 수락 main `ae8fc1fff9e4a964da412bba05cc9c3111d1e85a` (PR #1–39, 5개 워크플로 23개 잡 성공). main CI 성공은
...
진행 중(미수락):
- 협업 room 용량(`rust-collab-engine-capacity`): main은 여전히 `max_rooms.clamp(1, 4)`라 환경변수로 4를 넘지
  못한다. 브랜치의 64-room 목표(p95 ≤ 300 ms·writer 손실 0·1011 0·hostile 5/5)는 **미검증**이다(지연 원인 분석 중).
- 태스크 마일스톤·의존성(#40, migration 017, 검토 중).
...
| 그룹·권한 통합 | policies.ts effectivePermission, project/document_members(user XOR group) | 리소스별 단일 권한 함수 | 수락 | #23, #39 | 후속: 그룹 join 중복 정리·collab 프레임당 권한 재조회 축소·설정 UI `canManage` 불일치 |
| 프로젝트 | domains/projects | 비공개 접근(workspace admin 제외)·lead/멤버 제거 경합·원자성 | 부분 | #13, #39 | 미착수: 프로젝트 홈·프로젝트 문서·복제·lead 선택 UI |
| 태스크 | domains/tasks, core/task.ts, workflow.ts | 권한·버전·WIP·반복 회차 원자성·키셋 커서 | 부분 | #13, #19, #26, #38 | 진행: 마일스톤·의존성(#40). 미착수: activity feed·태스크 댓글 UI·검색 가능한 parent 선택 |
...
| 댓글 | comments/routes.ts, core/comment.ts | 문서 XOR 태스크·부모 활성·권한 | 부분 | #28 | 미착수: 태스크 댓글 UI, 그룹 멘션, 프로젝트 문서 댓글. 후속: 이중 DELETE 중복 이벤트·resolve 경합·동시 trash된 태스크 댓글 |
...
| 검색·색인·AI | domains/search, packages/search | 검색에서도 인가·철회·색인 복구 | 부분 | #29, #30, #35 | 수락: 워크스페이스 검색·outbox 색인·복구 후 rebuild. 미착수: 전역 GET /search, 의미(벡터) 검색, 댓글 검색 hit, 검색 E2E. 후속: partial 추출 chunk, 협업 편집마다 첨부 chunk 재색인(쓰기 증폭) |
| 알림·outbox·메일·webhook·연동 | domains/notifications, packages/jobs | 커밋 후 전달·중복/재시도 | 부분 | #31(relay), #35(첫 소비자) | 미착수: 알림·메일·webhook·연동 소비자 전체, requeue 운영 API/UI |
