...
최신 수락 main `8b858b42c43cdaf2eeb204a7c6c29d7e7508d0d1` (PR #1–30).
...
진행 중(미수락): outbox relay(#31, 차단 3건 수정 중), 검색 색인(outbox 위, `rust-search-index`),
협업 room 용량(메모리 예산 admission, `rust-collab-engine-capacity`), 백업·복구(#32 검토 중),
PAT(`rust-api-tokens`, outbox 다음 migration 013).
...
| --- | --- | --- | --- | --- | --- |
| 설치·로그인·세션·프로필 | identity/routes.ts, core/auth.ts | 활성 사용자, 철회, 본문+이벤트+감사 원자성 | 부분 | #1 | PAT(구현 완료·통합 대기), OIDC·MFA·비밀번호 재설정·계정 생명주기·설정 기반 비밀번호 정책 |
...
| 협업 | domains/collab, React/Tiptap | provider envelope·철회·CRDT 정본·persist barrier·writer generation·재시작 복원 | 부분(opt-in) | #6, #7, #18, #24, #27 | 실제 OS IME, 기존 데이터 전체 호환, **room 용량**(room당 helper 프로세스·기본 4 room/서버, 원본 대비 제약 → 메모리 예산 admission 진행 중), opt-in 해제 조건 |
...
| 검색·색인·AI | domains/search, packages/search | 검색에서도 인가·철회·색인 복구 | 부분 | #29, #30 | 사용자 결정: Meilisearch 원본 동등. 색인(outbox 소비자·rebuild) 진행 중, 전역 GET /search, 의미(벡터) 검색, 댓글 검색 hit, 검색 E2E |
| 알림·outbox·메일·webhook·연동 | domains/notifications, packages/jobs | 커밋 후 전달·중복/재시도 | 진행 | #31(outbox, 차단 수정 중) | 알림·메일·webhook·연동 소비자 전체 |
...
| 설치·배포 산출물 | infra/app, compose | 비특권 실행·migrate/grant 분리·helper 포함 | 부분 | #17, #20, #29 | 백업·복구(#32 검토 중), 복구 후 검색 rebuild, 운영 TLS/secure cookie 안내, 업그레이드 경로 |
...
- 협업 room 용량: 서버당 기본 4 room, helper 8개 즉시 거부. 원본 대비 제약이며 수정 진행 중.
