...
2. 각 `.github/workflows/{web,rust,documents,collab-engine,install}.yml` 의 `ci-plan` 은 pinned PyYAML(`scripts/ci_selection_requirements.txt`)을 설치한 뒤 `plan` 을 돌리고, `*-ci-gate` 는 항상 실행된다. 게이트는 `NEEDS_JSON=${{ toJSON(needs) }}` 와 tested SHA만 받으며 ci-plan 결과·plan_json·등록 job 결과를 그 객체에서 도출한다. 제품 job 은 plan 출력 boolean 으로만 skip 한다. matrix job 은 job-level `if` 로 통째로 skip 하며 빈 matrix 를 만들지 않는다.
