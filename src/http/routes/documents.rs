...
pub(crate) async fn export_document(
    ...
    // Every format is written by this binary's `--internal-markdown` child.
    ...
    let rendered = render_document_export(markdown, format, &meta.title, &meta.content_json).await;
