...
JOB_RESULT_RE = re.compile(r"--job-result(?:\s+|=)([A-Za-z0-9_-]+)=")
WORKFLOW_FLAG_RE = re.compile(r"--workflow(?:\s+|=)([A-Za-z0-9_-]+)")
...
def verify_workflow_registry(repo_root: Path = ROOT) -> list[str]:
    ...
    for workflow, filename in WORKFLOW_YAML.items():
        ...
        if isinstance(gate_job, dict):
            ...
            gate_runs = "\n".join(_run_scripts(gate_job))
            if "scripts/ci_selection.py gate" not in gate_runs:
                errors.append(f"{workflow}: {reserved_gate} must invoke ci_selection.py gate")
            result_jobs = JOB_RESULT_RE.findall(gate_runs)
            if set(result_jobs) != set(expected):
                errors.append(
                    f"{workflow}: {reserved_gate} --job-result arguments must match registered jobs"
                )
            workflows_passed = WORKFLOW_FLAG_RE.findall(gate_runs)
            if workflow not in workflows_passed:
                errors.append(f"{workflow}: {reserved_gate} must pass --workflow {workflow}")
            if "--plan-json" not in gate_runs:
                errors.append(f"{workflow}: {reserved_gate} must pass --plan-json")
