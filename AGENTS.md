...
| 주 구현 워커 | cursor-agent / Composer 2.5 (`composer-2.5`, 별도 effort 없음) | 배정된 Rust·프론트엔드 구현, 국소 리팩터링, 관련 회귀 |
| 조사·검증·독립 구현 워커 | cursor-agent / Grok 4.6 High (`cursor-grok-4.6-high`, 별도 effort 없음) | 계약 조사, 재현·호환성·부하·교차 검증, 소유권이 겹치지 않는 구현 |

**현재 배정 (2026-09-27 사용자 지시):** 이전 Fable·Opus 및 All-Opus 배정을 대체하며
이전 GPT sol 주 구현 배정도 대체하며 후속 세션과 하위 위임에도 유지한다.
새 구현·조사·테스트·실패 분석 워커는 위 두 Cursor 모델에 배정하고, GPT sol은 읽기 전용
자문·독립 검토만 맡는다. 두 Cursor 모델 사이의 분담은 조정할 수 있지만 같은 작업을 중복 구현하지 않는다.
GPT sol을 유사한 이름·버전으로 대체하지 않는다.
...
계산하지 않는다. 검토자는 제품 코드를 수정하지 않는다. Cursor 작성자의 자체·교차 검증은 필수 GPT sol
