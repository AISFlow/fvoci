...
| 자문 | Claude Code / Opus 5.5 / medium | 고정 SHA의 읽기 전용 설계·보안 검토 |
| 구현 워커 | cursor-agent / Composer 2.5 | 제품 구현과 관련 테스트 |
| 조사·검증 및 독립 구현 워커 | cursor-agent / Grok 4.6 | 원본 계약, 호환성 재현, 교차 검증, 배정된 구현 |

사용자의 최신 지시에 따라 모든 자문·독립 검토는 Claude Code Opus5.5 medium
(`claude-opus-5-5`)을 사용한다. Fable 한도 여부를 전환 조건으로 두지 않는다.
...
사용자의 2026-09-24 명시적 승인에 따라 코디네이터는 AISFlow/fvoci 작업 브랜치 일반 push, 기존/후속 PR 생성·갱신·Ready 전환 및 수락 후 머지를 반복 승인 없이 수행한다. 최신 HEAD의 필요한 로컬·실제 외부 시스템 검사, 실제 실행된 원격 CI, 필요한 Opus5.5 medium 독립 검토와 저장소 보호 조건을 모두 확인하고 기대 HEAD를 지정해 머지한다. 머지 후 기본 브랜치와 CI를 확인한 뒤 후속 기능 브랜치를 만든다.
