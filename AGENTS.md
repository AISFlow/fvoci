...
모델의 실제 ID와 실행 경로는 `.agents/environment.md`의 검증 기록을 따른다. Auto/Fast/다른 버전으로 대체하지 않는다. 단, 사용자 명시 승인에 따라 Claude Code Fable5.1의 실제 주간 한도 초과 시에만 Claude Code Opus5.5 medium을 사용할 수 있다. 전환 전 실제 모델 ID와 유효 설정을 확인하고 기존 검토 SHA·지적 사항을 이어받아 기록한다. 모델이 없으면 해당 위임을 차단으로 보고한다. 서브에이전트도 같은 모델·소유권·자원 규칙을 따른다.
